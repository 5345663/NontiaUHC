...
public class Scenario1 implements CustomInventory {
    ...
    public Supplier<ItemStack[]> getContents(Player player) {
        ...
        Stream.of(GuiScenarioEnum.values()).filter(scenario -> scenario.getPage() == 1).forEach(scenario -> slots[scenario.getSlot()] = new ItemCreator(scenario.getMaterial()).setName("§8◆ §e" + scenario.getName() + " §8(" + (scenario.isEnabled() ? "§aon" : "§coff") + "§8)").getItem());
        slots[49] = new ItemCreator(Material.BOOK).setName("§cModes de jeux").getItem();
        slots[53] = new ItemCreator(Material.PAPER).setName("§cPage 2").getItem();
