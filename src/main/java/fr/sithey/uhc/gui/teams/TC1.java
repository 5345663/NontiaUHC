...
public class TC1 implements CustomInventory {

    private Main main = Main.getInstance();

    @Override
    public String getName() {
        return "§6Teams §8(page 1)";
    }
    ...
    public Supplier<ItemStack[]> getContents(Player player) {
        ...
        slots [45] = new ItemCreator(Material.STAINED_GLASS_PANE).setDurability(14).setName("§8").getItem();
            ...
            slots [51] = new ItemCreator(Material.EMERALD).setName("§8Page 2").getItem();
    ...
    public void onClick(Player player, Inventory inv, ItemStack current, int slot) {
        switch (current.getType()) {
            case BANNER:
                for (Teams teams : Teams.values()) {
                        if (current.getItemMeta().getDisplayName().equalsIgnoreCase(teams.getColor()+teams.getName())) {
